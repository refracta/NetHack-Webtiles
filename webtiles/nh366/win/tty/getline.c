...
register char *bufp;
{
    ...
    send_debug("void tty_getlin(query:%s, bufp:%s)", stringify(query), stringify(bufp));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void xwaitforspace(s:%s)", stringify(s));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("boolean ext_cmd_getlin_hook(base:%s)", stringify(base));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_get_ext_cmd()");
    #endif
    int i;
    char buf[BUFSZ];
