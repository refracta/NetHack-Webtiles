...
tty_doprev_message()
{
    ...
    send_debug("int tty_doprev_message()");
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void redotoplin(str:%s)", stringify(str));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void show_topl(str:%s)", stringify(str));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void addtopl(s:%s)", stringify(s));
    #endif
    register struct WinDesc *cw = wins[WIN_MESSAGE];
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void update_topl(bp:%s)", stringify(bp));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void topl_putsym(c:%c)", c);
    #endif
    register struct WinDesc *cw = wins[WIN_MESSAGE];
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void putsyms(str:%s)", stringify(str));
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void removetopl(n:%d)", n);
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void msghistory_snapshot(purge:%d)", purge);
    #endif
    char *mesg;
    int i, inidx, outidx;
    struct WinDesc *cw;
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void free_msghistory_snapshot(purged:%d)", purged);
    #endif
    if (snapshot_mesgs) {
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("char * tty_getmsghistory(init:%d)", init);
    #endif
    static int nxtidx;
    char *nextmesg;
    char *result = 0;
...
boolean restoring_msghist;
{
    #if defined(WEBTILES_DEBUG)
    if(msg == NULL){
    send_debug("void tty_putmsghistory(msg:NULL, restoring_msghist:%d)", restoring_msghist);
    }else{
    send_debug("void tty_putmsghistory(msg:%s, restoring_msghist:%d)", stringify(msg), restoring_msghist);
    }
    #endif
