...
int *wid, *hgt;
{
    ...
    send_debug("void tty_startup(wid:?, hgt:?)");
    #endif
    register int i;
#ifdef TERMLIB
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_decgraphics_termcap_fixup()");
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_ascgraphics_hilite_fixup()");
    #endif
    register int c;
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_start_screen()");
    #endif
    xputs(TI);
    xputs(VS);
#ifdef PC9800
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_end_screen()");
    #endif
    clear_screen();
    xputs(VE);
    xputs(TE);
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void nocmov(x:%d, y:%d)", x, y);
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void cmov(x:%d, y:%d)", x, y);
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void tty_delay_output()");
    #endif
#if defined(MICRO)
    register int i;
#endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void analyze_seq(str:%s, fg:?, bg:?)", stringify(str));
    #endif
    register int c, code;
    int len;
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("char* s_atr2str(n:%d)", n);
    #endif
    switch (n) {
    case ATR_BLINK:
    case ATR_ULINE:
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("char* e_atr2str(n:%d)", n);
    #endif
    switch (n) {
    case ATR_ULINE:
        if (nh_UE && *nh_UE)
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("int term_attr_fixup(msk:%d)", msk);
    #endif
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_start_attr(attr:%d)", attr);
    #endif
    if (attr) {
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_end_attr(attr:%d)", attr);
    #endif
    if (attr) {
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_start_raw_bold()");
    #endif
    xputs(nh_HI);
}
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_end_raw_bold()");
    #endif
    xputs(nh_HE);
}
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_end_color()");
    #endif
    xputs(nh_HE);
}
...
{
    #if defined(WEBTILES_DEBUG)
    send_debug("void term_start_color(color:%d)", color);
    #endif
    if (color < CLR_MAX)
        xputs(hilites[color]);
}
